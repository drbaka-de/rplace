...
def update_from_upstream(original_url, filename):
	...
	for line in response.text.split("\n"):
		...
		if b64pngmatch:
			...
			img = Image.open(BytesIO(binascii.a2b_base64(b64pngmatch.group(1)))).convert("RGBA")
			if img.size[0] > 80:
	...
	if img:
		img_new = Image.new('RGBA', (img.size[0], img.size[1]))
		for x in range(img.size[0]):
			for y in range(img.size[1]):
				...
				img_new.putpixel((x, y), ((c>>16) & 0xff, (c>>8) & 0xff, c & 0xff, 0xff))
